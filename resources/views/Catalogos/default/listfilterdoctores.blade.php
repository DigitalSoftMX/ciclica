@extends('layouts.appusuarios', ['activePage' => 'category-management', 'menuParent' => 'laravel', 'titlePage' => ' '])
...
@section('content')
  <div class="content">
    ...
    <!-- CANJES-->
    @if($catalog->getUrlPrefix() == "laboratorio" || $catalog->getUrlPrefix() == "laboratoriofinalizado")
        <div class="row">
          <div class="col-sm-4">
              <div class="card">
                   @if($catalog->getUrlPrefix() == "laboratorio")
                    <ul class="nav nav-pills nav-pills-warning nav-pills-icons justify-content-center" role="tablist" style="background:#FFD003">
                    <li class="nav-item">
                    <a class="nav-link" href="{{ url("laboratorio/edit/$id") }}" >
                    @else
            ...
            <div class="col-sm-4">
              <div class="card">
                   @if($catalog->getUrlPrefix() == "laboratoriofinalizados")
                      <ul class="nav nav-pills nav-pills-warning nav-pills-icons justify-content-center" role="tablist" style="background:#FFD003">
                      <li class="nav-item">
                      <a class="nav-link" href="{{ url("laboratoriofinalizado/edit/$id") }}">
                    @else
                      <ul class="nav nav-pills nav-pills-warning nav-pills-icons justify-content-center" role="tablist">
                      <li class="nav-item">
                      <a class="nav-link" href="{{ url("laboratoriofinalizado/edit/$id") }}">
...
@include("Catalogos.forms.usuarios.laboratoriolist")
    ...
    $.each(fields,function(index,value){columns.push({'data':value});});
      ...
      $('#datatables').fadeIn(1100);
        $('#datatables').DataTable( {
            "processing": true,
            "responsive": true,
            "ajax": "https://ciclica.lealtaddigitalsoft.mx/public/laboratoriojlistl/{{$id}}",
...
<script>
  ...
  var parametros = { "min" : values1,
    ...
    fields = {!! json_encode( array_keys($catalog->field_list) ) !!};
    url_add = 'laboratorio/add/';
    url_delete = 'laboratorio/destroy/';
    url_edit = 'laboratorio/edit/';
    url_ver = 'laboratorio/ver/';
    columns = [];
    $.each(fields,function(index,value){columns.push({'data':value});});
      ...
      $('#datatables').fadeIn(1100);
        $('#datatables').DataTable( {
            ...
            "destroy": true, 
            "ajax": {
               "ajax": "https://ciclica.lealtaddigitalsoft.mx/public/laboratoriojfilter",
               "type": 'POST',
               "data": parametros,
             },
            "deferRender": true,
