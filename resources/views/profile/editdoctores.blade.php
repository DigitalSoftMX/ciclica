@extends('layouts.appdoctores', ['activePage' => 'profile', 'menuParent' => 'laravel', 'titlePage' => __('Perfil')])
                  ...
                  <div class="fileinput fileinput-new text-center" data-provides="fileinput">
                    <div class="fileinput-new thumbnail img-circle">
                      @if (auth()->user()->picture)
                        <img src="https://ciclica.lealtaddigitalsoft.mx/storage/app/public/{{ auth()->user()->picture }}" />
                      @else
               ...
               <div class="row">
                ...
                <div class="col-sm-7">
                  <div class="form-group{{ $errors->has('app_name') ? ' has-danger' : '' }}">
                    <input class="form-control{{ $errors->has('app_name') ? ' is-invalid' : '' }}" name="app_name" id="input-name" type="text" placeholder="{{ __('Apellido Paterno') }}" value="{{ old('name', auth()->user()->app_name) }}" required="true" aria-required="true"/>
               ...
               <div class="row">
                ...
                <div class="col-sm-7">
                  <div class="form-group{{ $errors->has('apm_name') ? ' has-danger' : '' }}">
                    <input class="form-control{{ $errors->has('apm_name') ? ' is-invalid' : '' }}" name="apm_name" id="input-name" type="text" placeholder="{{ __('Apellido Materno') }}" value="{{ old('name', auth()->user()->apm_name) }}" required="true" aria-required="true"/>
                    @include('alerts.feedback', ['field' => 'apm_name'])
                  </div>
                </div>
              </div>
               <div class="row">
                ...
                <div class="col-sm-7">
                  <div class="form-group{{ $errors->has('phone') ? ' has-danger' : '' }}">
                    <input class="form-control{{ $errors->has('phone') ? ' is-invalid' : '' }}" name="phone" id="input-name" type="text" placeholder="{{ __('Telefono') }}" value="{{ old('name', auth()->user()->phone) }}" required="true" aria-required="true"/>
          ...
          <div class="card-body">
            <form method="post" action="{{ route('profile.password') }}" class="form-horizontal">
              ...
              <div class="row">
               ...
               <button type="submit" class="btn btn-rose pull-right">Cambiar password</button>
              <div class="clearfix"></div>
            </form>
          </div>
        </div>
      ...
      <div class="col-md-4">
        <div class="card card-profile">
          <div class="card-avatar">
            <a href="#pablo">
              <img class="img" src="https://ciclica.lealtaddigitalsoft.mx//storage/app/public/{{ auth()->user()->picture }}" />
            </a>
          </div>
          <div class="card-body">
        <h4 class="col-sm-12 text-center card-title">Especialidades</h4>
              @forelse ($especiali as $product)
                    <label class="col-sm-8 col-form-label text-center">{{ $product->espe }}</label>
                    </br>
                @empty
                 <div class="card-header card-header-primary text-center">
                     <h4>No se tiene ligado ninguna especialidad</h4>
                 </div>
                @endforelse
          </div>
        </div>
